...
This section closely tracks with content from the book [R Packages] by Hadley Wickham and Jenny Bryan [-@wickham-unpub], specifically the [The Whole Game] chapter. The material in this chapter is, however, accurate and up-to-date, although a bit longer than [The Whole Game]. The [R Packages] book is a great place to go next to learn more. 
...
We use the [devtools] package to facilitate package development. It’s on [CRAN][devtools-cran] and developed on [GitHub][devtools-github]. Why devtools? 
...
For presentation purposes only, we use the [tidyverse], for light data wrangling, and [fs], for filesystem work.
...
We call the package foofactors and it will have a couple functions for handling factors. Please note that these functions are super simple and definitely not the point! For a proper package for factor handling, please see the [forcats] package.
...
What files are in this new directory? Here's a listing using the [fs] package (locally, you can consult your Files pane):
...
* Read more in the [Package metadata](https://r-pkgs.org/description.html) chapter of [R Packages] [@wickham-unpub]
...
There is more than one way to go about this. We use the approach recommended in the [Namespace][r-pkgs-namespace] chapter of [R Packages] [@wickham-unpub] and in the [rOpenSci Packaging Guide](https://ropensci.github.io/dev_guide/building.html#package-dependencies).  The general usage pattern is this: give the name of the external package, then two colons, then the function to call.
...
A piece of package-level documentation that's probably familiar to you is the vignette. This is a great place to put a fully developed example that calls multiple functions from your package to do something useful and realistic. See the [Vignettes](https://r-pkgs.org/vignettes.html) chapter of [R Packages] [@wickham-unpub] for more.
...
*See the [Cache credentials for HTTPS](https://happygitwithr.com/credential-caching.html) chapter of [Happy Git and GitHub for the useR].*
...
Now that your package is on GitHub, the `README.md` file matters. It is the package's home page and welcome mat, at least until you decide to give it a website (see [pkgdown]) or submit it to CRAN (see the [Releasing a package](https://r-pkgs.org/release.html) chapter in the [R Packages] book [@wickham-unpub]).
...
[r-pkgs-namespace]: https://r-pkgs.org/namespace.html
