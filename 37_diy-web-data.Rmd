...
Our JSON response above can be parsed using the `fromJSON()` function in the [jsonlite] package. First install/load jsonlite.
...
1. The [rvest] package ([CRAN](https://CRAN.R-project.org/package=rvest); [GitHub](https://github.com/tidyverse/rvest)). Install via `install.packages("rvest)"`.
