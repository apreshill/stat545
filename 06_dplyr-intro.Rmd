...
Before we go any further, we should exploit the new pipe operator that the tidyverse imports from the [magrittr][4] package by Stefan Bache. This is going to change your data analytical life. You no longer need to enact multi-operation commands by nesting them inside each other, like so many [Russian nesting dolls][5]. This new syntax leads to code that is much easier to write and to read.
...
dplyr's verbs, such as `filter()` and `select()`, are what's called [pure functions][6]. To quote from Wickham's [Advanced R Programming book][7]  [-@wickham2015a]:
...
This set of deliberate design choices, together with the new pipe operator, produces a highly effective, low friction [domain-specific language][8] for data analysis.
...
[3]: https://www.jstatsoft.org/article/view/v040i01
[4]: https://magrittr.tidyverse.org
[5]: https://en.wikipedia.org/wiki/Matryoshka_doll
[6]: https://en.wikipedia.org/wiki/Pure_function
[7]: http://adv-r.had.co.nz/Functions.html
[8]: http://adv-r.had.co.nz/dsl.html
